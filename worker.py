...
from shared import simulate_pnl
...
# =========================
# Supabase (UI uses Streamlit secrets)
# =========================
@st.cache_resource
def supabase_client() -> Client:
    url = st.secrets.get("SUPABASE_URL")
    key = st.secrets.get("SUPABASE_SERVICE_KEY")  # ok för nu; senare kan du byta till anon + RLS
...
st.markdown(
    ...
    <div class="card">
      ...
      <div class="small">UI reads from Supabase • Worker writes calls continuously</div>
    </div>
    """,
    unsafe_allow_html=True
)

# Top row: Export + status
...
with coly:
    st.markdown("<div class='small neutral'>Tip: Leave worker running 24/7. UI can sleep; data still logs.</div>", unsafe_allow_html=True)
...
# Simple refresh (doesn't block forever with while loop)
st.caption(f"Auto-refresh every ~{REFRESH_SECONDS}s.")
st.rerun if False else None  # no-op placeholder
st.autorefresh = None

# Optional: use Streamlit's built-in autorefresh component if installed
# If not, just hit refresh in browser.
try:
    from streamlit_autorefresh import st_autorefresh
    st_autorefresh(interval=REFRESH_SECONDS * 1000, key="ui_refresh")
except Exception:
    st.info("Install streamlit-autorefresh for automatic refresh: pip install streamlit-autorefresh")
